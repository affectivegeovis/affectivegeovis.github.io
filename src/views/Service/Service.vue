  ...
  <div class="filter-bar" style="text-align: left;margin-top: 3%;margin-bottom: 0%;font-size: 1rem;">
    <p>
        ...
        <i class="fa-regular fa-comment-dots" style="color:black;padding: 0; margin-left: 5px;font-size: small"></i>: other blogs/web materials).
         ...
         <section class="Articles_corpus" id="Articles_corpus" :style="{ 'padding': '0px'}">
         <ul class="cards" >
            ...
            <li class="card" v-for="(item, index) in filteredData" :key="index">
             ...
             <img 
               v-if="item.isImageLoaded" 
               :src="require('@/assets/images/each/'+item.images)" 
               alt="img"  
             ...
             <img 
               v-else 
               :src="require('@/assets/images/each/Loading.jpg')" 
               alt="LOADINGing" 
               style="aspect-ratio: 16 / 9;"
               loading="lazy"
               :data-src="require('@/assets/images/each/'+item.images)"
               :ref="'lazyImage-' + index"
             >
             <p>
                 ...
                 setupLazyLoading() {
                         ...
                         entries.forEach(entry => {
                             if (entry.isIntersecting) {
                                 ...
                                 if (src) {
                                     ...
                                     const index = img.getAttribute('ref').split('-')[1];
                                     this.jsonData[index].isImageLoaded = true;
                                 }
                     ...
                     this.$nextTick(() => {
                         this.filteredData.forEach((item, index) => {
                             const imgRef = this.$refs[`lazyImage-${index}`];
                             if (imgRef) {
